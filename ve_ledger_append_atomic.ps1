...
#  - Stable UTF-8 hashing (BOM-safe)
#  - Optional fixed-point basis points fields (*_bps) while preserving legacy floats
#  - No locale-dependent formatting
#  - Deterministic, stable JSON serialization for hashing
...
function Convert-ToStableJson {
    ...
    # Prefer System.Text.Json when available (PowerShell 7+ / .NET Core)
    $stjType = [type]::GetType("System.Text.Json.JsonSerializerOptions", $false)
    if ($stjType) {
    ...
    # Fallback: Newtonsoft.Json for Windows PowerShell 5.1
    try {
        Add-Type -AssemblyName "Newtonsoft.Json" -ErrorAction Stop | Out-Null
    } catch {
        throw "Stable JSON serializer unavailable. Install Newtonsoft.Json or run under PowerShell 7+. Details: $(# ve_ledger_append_atomic.ps1
# Purpose: Append ledger entry with hash chain using atomic write + named mutex
# Patch posture (v1.0.3):
#  - Stable UTF-8 hashing (BOM-safe)
#  - Optional fixed-point basis points fields (*_bps) while preserving legacy floats
...
function Convert-ToStableJson {
    ...
    # System.Text.Json gives consistent output; relaxed escaping aligns with ensure_ascii=False intent.
    $opt = [System.Text.Json.JsonSerializerOptions]::new()
    $opt.WriteIndented = $false
    $opt.Encoder = [System.Text.Encodings.Web.JavaScriptEncoder]::UnsafeRelaxedJsonEscaping
...
function To-Bps {
    param([double]$x)
    # Clamp into [0,1], then round to nearest basis point
    $c = Clamp-01 $x
...
function Round2 {
    param([double]$x)
    # Deterministic rounding independent of locale
...
try {
    ...
    # Decide whether to use bps as source of truth:
    ...
    if ($haveBps) {
        ...
        # Legacy floats provided; compute bps only if EmitBps is set
        $rho_bps   = [Nullable[int]](To-Bps $rho)
        $gamma_bps = [Nullable[int]](To-Bps $gamma)
        $delta_bps = [Nullable[int]](To-Bps $delta)
    }

    # Canonicalize floats to 2 decimals (keeps your legacy “N2” intent without locale issues)
    $rho2   = Round2 (Clamp-01 $rho)
    $gamma2 = Round2 (Clamp-01 $gamma)
    $delta2 = Round2 (Clamp-01 $delta)

    # Build ordered entry for deterministic JSON key order
    $entry = [ordered]@{
        time      = (Get-Date).ToString("s")
        type      = $etype
        actor     = $actor
    ...
    # Optional bps emission (Patch posture)
    ...
    # Stable hash of JSON excluding hash_self (since it isn't present yet)
...
.Exception.Message)"
    ...
    # Do not force ASCII escapes; allow UTF-8 characters to remain (aligns with ensure_ascii=False intent)
